...
<html>
<head>
    <title>Kartu Hasil Studi <?= $tampil_mhs['nama_mhs']; ?></title>
    <style>
        body {
  ...
  <table class="basic" width="750" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr>
      <td align="center" class="style7">
        <h2 style="font-size: 17px;">KARTU HASIL STUDI (KHS)</h2>
        </td>
      </tr>
    </table>
    <table class="table_1">
    ...
    <table class="table_2">
     ...
     if (isset($_GET['qwe'])) {
        ...
        <tr>
          ...
          <td align="center"><?= $t_khs['kode_mk']; ?></td>
          <td><?= $t_khs['nama_matkul']; ?></td>
          <td align="center"><?= $t_khs['sks']; ?></td>
          <td align="center"><?= $t_khs['semester']; ?></td>
          <td><?= $t_khs['nama_dosen']; ?></td>
          <td align="center"><?= $t_khs['nilai_akhir']; ?></td>
          <td align="center"><?= $t_khs['grade']; ?></td>
        </tr>
      <?php }
         } ?>
    </table>
...
if ($cek_data > 0) {
    ...
    <tr>
      <td valign="top">Indeks Prestasi Semester (IPS)</td>
      <td valign="top">
        <?php
        $cek_data = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM khs_mhs WHERE nim_npm='$username' AND kode_prodi='$kode_prodi' AND id_thn_akademik='$id_thn_akademik'"));
if ($cek_data > 0) {
...
<table class="hk">
    ...
    <tr>
      <td width="430" align="center" style="font-size: 12px;">Menyetujui &amp; Mengetahui</td>
      <td width="350"></td>
      <td align="center" style="font-size: 12px;"><?= $r_pengaturan['kota']; ?>, <?= date('d F Y'); ?></td>
    </tr>
    <tr>
      <td width="530" align="center" style="font-size: 12px;">Penasehat Akademik<br /><br /><br /><br /><br />
        <?php
         $pa = mysqli_fetch_array(mysqli_query($koneksi, "SELECT * FROM mhs_has_pa INNER JOIN dosen ON mhs_has_pa.nip=dosen.nip WHERE nim_npm='$username'"));
?>
