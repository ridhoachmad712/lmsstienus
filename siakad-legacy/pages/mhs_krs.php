...
$nim_npm = siakad_scoped_advisee_username($koneksi, $_GET['qaz']);
//
...
?>
      ...
      if (isset($_POST['filter']) or (isset($_GET['qwe']))) {
        ...
        <div class="card-body">
          <table>
            <tr>
              <td>
                <!-- <a class="btn btn-yellow" target="_blank" href="cetak/krs?qwe=<?= $id_thn_akademik; ?>" style="text-decoration: none;">Cetak</a> -->
              </td>
              <td>
               <?php
          ...
          if ($dari_tgl == '0000-00-00') {
              ?>
                <button class="btn btn-secondary disabled">Tambah KRS</button>
              <?php } else { ?>
    ...
    if (isset($_POST['filter']) or (isset($_GET['qwe']))) {
      ...
      <?php } else { ?>
       <div class="alert btn-dark">
        <p><i>Perhatian !!!</i><br>
          Jadwal Penawaran Mata kuliah dimulai <b style="color: yellow;">(tanggal <?= tgl_indo($dari_tgl); ?> dan berakhir tanggal <?= tgl_indo($jadwal_menawar['sampai_tgl']); ?>)</b> mohon diperhatikan untuk mahasiswa</p>
        </div>
      <?php }
      } ?>
      <!-- tampil data -->
        ...
        <tbody>
          <?php
            if (isset($_POST['filter'])) {
              ...
              LEFT JOIN dosen ON jadwal_mengajar.nip=dosen.nip WHERE krs_mhs.nim_npm='$nim_npm' AND krs_mhs.id_thn_akademik='$id_thn_akademik' AND krs_mhs.kode_prodi='$kode_prodi' ORDER BY jadwal_mengajar.id_hari ASC");
              ...
              <tr>
                <?php
                  ...
                  <?php } else { ?>
                    ...
                    <td><?= $t_krs['sks']; ?></td>
                    <td><?= $t_krs['nama_ruangan']; ?> - Lantai <?= $t_krs['lantai']; ?></td>
                    <td><?= $t_krs['semester']; ?></td>
                    <td><?= $t_krs['nama_hari']; ?> - <?= $t_krs['mulai_jam']; ?> - <?= $t_krs['sampai_jam']; ?></td>
                   ...
                   <?php
                    ...
                    <table>
                      <tr>
                        ...
                        <th>:
                          <?php
                                       $cek_data = mysqli_num_rows(mysqli_query($koneksi, "SELECT * FROM krs_mhs WHERE nim_npm='$nim_npm' AND kode_prodi='$kode_prodi' AND id_thn_akademik='$id_thn_akademik'"));
                                 if ($cek_data > 0) {
                                     ?>
                           <?php
                                     $t_sks = mysqli_query($koneksi, "SELECT * FROM krs_mhs INNER JOIN
                            jadwal_mengajar ON krs_mhs.id_jadwal=jadwal_mengajar.id_jadwal
                            LEFT JOIN mata_kuliah ON jadwal_mengajar.kode_mk=mata_kuliah.kode_matkul WHERE nim_npm='$nim_npm' AND krs_mhs.kode_prodi='$kode_prodi' AND krs_mhs.id_thn_akademik='$id_thn_akademik'");
                    ...
                    <tr>
                      ...
                      <th>:
                        <?php
                        $sks_diambil = mysqli_fetch_array(mysqli_query($koneksi, "SELECT * FROM pengaturan_sks_mhs WHERE id_thn_akademik='$id_thn_akademik' AND nim_npm='$nim_npm'"));
                                 echo $sks_diambil['sks'];
                                 ?>
                      </th>
                    </tr>
                    <tr>
